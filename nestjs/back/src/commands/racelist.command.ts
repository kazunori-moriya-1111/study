...
export class RecelistCommand extends CommandRunner {
  ...
  constructor(
    ...
    super();
    this.logger = this.loggerFactory.createLogger(RecelistCommand.name);
  }
  ...
  async run(inputs: string[], options?: CommandOptions): Promise<void> {
    ...
    const response = await fetch(
      `https://www.boatrace.jp/owsp/sp/spdata?hd=${date}&jcd=${jid}&type=racelist`,
    );
    const json = await response.json();
    this.logger.log(`created : ${JSON.stringify(json)}`);

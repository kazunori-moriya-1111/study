...
describe('ItemServiceTest', () => {
  ...
  beforeEach(async () => {
    ...
    itemRepository = module.get<Repository<Item>>(
      getRepositoryToken(Item),
    ) as jest.Mocked<Repository<Item>>;
  });
  ...
  describe('findById', () => {
    ...
    it('異常系: 商品が存在しない', async () => {
      ...
      await expect(itemsService.findById('testId')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('create', () => {
    it('正常系', async () => {
      ...
      jest.spyOn(itemRepository, 'create').mockReturnValue(expected);
  ...
  describe('updateStatus', () => {
    ...
    it('異常系: 自身の商品を購入', async () => {
      jest
        ...
        .mockImplementation(async () => mockItem);
      await expect(
        itemsService.updateStatus('testId', mockUser1),
  ...
  describe('delete', () => {
    ...
    it('異常系: 他人の商品を削除', async () => {
      jest
        .spyOn(itemRepository, 'findOneBy')
        .mockImplementation(async () => mockItem);
      await expect(itemsService.delete('testId', mockUser2)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
